...
execute if score #lodestone_teleport lodestone_teleport.c.notify_chunkload matches 1.. run tellraw @a[tag=lodestone_teleport.TeleportHelper] [{"text": "[lodestone_teleport] ","color": "gold"},{"text": "Force loading target chunk!","color": "red"}]
# The macro function teleport
$execute in $(dimension) run forceload add $(x) $(z)
# Chunkloading complete message
execute if score #lodestone_teleport lodestone_teleport.c.notify_chunkload matches 1.. run tellraw @a[tag=lodestone_teleport.TeleportHelper] [{"text": "[lodestone_teleport] ","color": "gold"},{"text": "Chunk loading complete!","color": "green"}]
